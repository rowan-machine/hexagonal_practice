"""
Parameterized Airflow DAG for policies pipeline.
...
def run_policies_pipeline(**context) -> Dict[str, Any]:
    """
    Run policies pipeline with optional parameters.
    ...
    run_id = conf.get('run_id') or f"policies_{execution_date.strftime('%Y%m%d_%H%M%S')}"
...
dag = DAG(
    ...
    max_active_runs=1,
    tags=['stop-loss', 'insurance', 'policies', 'data-pipelines'],
    params={
