"""
Backfill DAG for policies pipeline.

Use this DAG to backfill historical policies data.
...
from src.pipelines import PoliciesPipeline
...
def run_policies_backfill(**context) -> Dict[str, Any]:
    """
    Run policies pipeline for backfill.
    ...
    run_id = conf.get('run_id') or f"policies_backfill_{execution_date.strftime('%Y%m%d_%H%M%S')}"
    
    print(f"Running policies backfill for date: {execution_date}")
...
dag = DAG(
    ...
    description='Backfill DAG for policies pipeline - processes historical data',
    ...
    max_active_runs=1,
    tags=['stop-loss', 'insurance', 'policies', 'backfill', 'data-pipelines'],
    params={
...
backfill_task = PythonOperator(
    task_id='run_policies_backfill',
    python_callable=run_policies_backfill,
    dag=dag,
)
